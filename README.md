...
这个项目的出发点，是让已经退出日常使用的老旧 Android 手机和平板继续发挥余热。只要设备还能连接局域网、正常显示画面，就可以把它变成常驻桌面的电脑状态屏、万年历时钟、应用启动面板和电子相册，而不必因为系统版本较低、性能有限就让它长期闲置。

Windows 端负责采集数据并通过 SSE 实时推送，Android 端自动发现同一局域网中的电脑。整个系统在本地运行，不依赖云服务、在线账号或外部服务器，既降低老设备的运行负担，也让监控数据和家庭照片留在自己的局域网内。

## 为什么做这个项目

- **盘活旧设备**：把闲置手机和平板变成长期有用的信息终端，而不是电子垃圾
- **兼容老系统**：Android 客户端最低支持 Android 5.0（API 21），重点照顾 Android 5–7 设备
- **功能不止监控**：在电脑状态面板之外，提供大时钟、万年历、应用启动器和电子相册
- **本地优先**：数据采集、设备发现和图片传输均在局域网内完成，不要求注册账号
...
本项目的代码、界面和文档全程由 AI 编写与迭代，维护者负责提出需求、确定产品方向、在真实 Windows 电脑和老旧 Android 设备上测试，并对最终效果进行验收。

这也是本项目的一项实践目标：探索 AI 是否能够从需求讨论开始，持续完成跨 Go、Web、Windows GUI 和 Android 原生开发的完整项目。AI 生成并不代表代码天然正确；欢迎审阅实现、报告问题并提交改进。
